...
const SYMPTOMS_LIST = [
    ...
    "Sudden Weakness", "Difficulty Speaking", "High Fever (>103°F)", "Back Pain",
    "Joint Pain", "Stomach Pain", "Acid Reflux", "Skin Rash", "Acne", "Headache",
    "Dizziness", "Anxiety", "Depression", "Cough", "Fever"
];
            ...
            <div className="min-h-[300px]">
                ...
                {step === 2 && (
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <Label className="text-base font-semibold">Select all symptoms that apply:</Label>
                ...
                {step === 4 && result && (
                    <div className="space-y-6 animate-in zoom-in-95 duration-300">
                        <Alert variant={result.severity === 'EMERGENCY' || result.severity === 'HIGH' ? "destructive" : "default"}>
                            <div className="flex items-center gap-2 mb-2">
                                {result.severity === 'EMERGENCY' ? <AlertCircle className="w-5 h-5" /> : <CheckCircle2 className="w-5 h-5 text-wellness" />}
                                <AlertTitle className="text-lg mb-0 font-bold">Assessment: {result.severity}</AlertTitle>
                            </div>
                            <AlertDescription>
                                Based on your inputs, we've categorized your case as <strong>{result.severity}</strong>.
                                {result.severity === 'EMERGENCY' && " Please seek immediate medical attention or visit the emergency department."}
                            </AlertDescription>
                        </Alert>

                        <div className="p-6 rounded-xl bg-secondary/20 border-2 border-primary/20 space-y-4">
                            <h3 className="font-bold text-lg flex items-center gap-2 text-primary">
                                <Stethoscope className="w-5 h-5" />
                                Our Suggestion
                            </h3>
                            <div className="space-y-2">
                                <p className="text-sm">We recommend booking an appointment with a:</p>
                            ...
                            {result.isEscalated ? (
                                <div className="p-3 bg-wellness/10 border border-wellness/20 rounded-lg">
                                    <p className="text-xs text-wellness font-bold">✓ ESCALATED TO ADMIN DOCTOR</p>
                                    <p className="text-[10px] text-muted-foreground mt-1">Due to the severity of your symptoms, you are eligible to book directly with our Admin/Primary doctor.</p>
                                </div>
                            ) : (
                                <div className="p-3 bg-secondary/30 rounded-lg">
                                    <p className="text-xs font-semibold">Specialist Only</p>
                                    <p className="text-[10px] text-muted-foreground mt-1 text-balance">The Admin Doctor is currently reserved for serious cases. A {result.suggestedSpecialty} will be better suited for your initial evaluation.</p>
                                </div>
                            )}
                        </div>
                    </div>
