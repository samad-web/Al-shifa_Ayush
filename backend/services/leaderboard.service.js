...
export class LeaderboardService {
     ...
     */
    static async calculateParticipantScore(participantId, role) {
        ...
        allSourceIds.push(...appointments.map(a => a.id));

        // 2. Adherence Rate Metric
        const clinician = role === 'THERAPIST'
        ...
        const { avgMinutes, sourceIds: responseTimeIds } = await this._calculateResponseTimeMetric(participantId, thirtyDaysAgo);
        ...
        const totalJourneys = clinician?.journeys?.length || 0;
        const completedJourneys = clinician?.journeys?.filter(j => j.status === 'COMPLETED').length || 0;
        ...
        const { consistency, sourceIds: consistencyIds, activeDaysCount } = await this._calculateConsistencyScore(participantId, thirtyDaysAgo);
    ...
    static async getLeaderboard() {
        const [doctors, therapists] = await Promise.all([
            ...
            prisma.therapist.findMany({ include: { user: true } })
        ]);
        ...
        const rankedParticipants = await Promise.all(participants.map(async (p) => {
            const result = await this.calculateParticipantScore(p.id, p.role);

            // Get last audit for trend
        ...
        return rankedParticipants.sort((a, b) => b.score - a.score);
    }
    ...
    static async getParticipantBreakdown(participantId) {
        ...
        if (!latestAudit) throw new Error('No performance data found for this participant');
     ...
     */
    static async _calculateResponseTimeMetric(participantId, thirtyDaysAgo) {
        const sourceIds = [];
        let totalMinutes = 0;
        let count = 0;

        // 1. Clinical Response: TriageSession to Appointment
        const appointmentsWithTriage = await prisma.appointment.findMany({
            where: {
                OR: [{ doctorId: participantId }, { therapistId: participantId }],
     ...
     */
    static async _calculateConsistencyScore(participantId, thirtyDaysAgo) {
        const sourceIds = [];
